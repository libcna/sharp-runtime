...
2. **No test-count regression.** `scripts/run_component_tests.sh build` must show no failures. The verified baseline is 12,635 tests across 36 component executables and one integration executable; this floor should be raised as new tests are added and lowered only with an explicit, documented reason.
