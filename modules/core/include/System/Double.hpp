...
private:
    ...
    static bool tryParseCore(const std::string& s, double& result) noexcept {
        ...
        // .NET's Double.Parse defaults to NumberStyles.Float | AllowThousands
        // (Double.cs:388), and std::from_chars has no grouping mode, so a ',' is
        // validated and removed here first. Nothing is allocated unless the input
        ...
        if (ec == std::errc::result_out_of_range && ptr == last && ptr != first) {
            // A magnitude outside double's range is not a format error in .NET:
