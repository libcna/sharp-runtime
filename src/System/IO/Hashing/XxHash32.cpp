...
uint32_t XxHash32::rotl32(uint32_t v, int n) { return (v << n) | (v >> (32 - n)); }
...
void XxHash32::processBlock(const uint8_t* block) {
    uint32_t lane;
...
XxHash32::XxHash32(uint32_t seed)
    : NonCryptographicHashAlgorithm(4), seed_(seed)
{
    initState();
}
...
void XxHash32::Append(const uint8_t* source, size_t length) {
    totalLength_ += length;
...
void XxHash32::GetCurrentHash(uint8_t* dest, size_t /*len*/) {
    uint32_t h32;
    if (totalLength_ >= 16) {
...
uint32_t XxHash32::GetCurrentHashAsUInt32() {
    uint8_t buf[4];
    GetCurrentHash(buf, 4);
    uint32_t r; std::memcpy(&r, buf, 4); return r;
}

uint32_t XxHash32::HashToUInt32(const std::vector<uint8_t>& source, uint32_t seed) {
    XxHash32 h(seed);
    h.Append(source);
