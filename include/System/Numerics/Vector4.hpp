...
struct Vector4 {
    ...
    /// @return The component at @p i (0=X,1=Y,2=Z,3=W). Throws std::out_of_range on invalid index.
    float operator[](int i) const {
        if(i==0)return X; if(i==1)return Y; if(i==2)return Z; if(i==3)return W;
        throw std::out_of_range("index");
    }
    /// @return A reference to the component at @p i (0=X,1=Y,2=Z,3=W). Throws std::out_of_range on invalid index.
