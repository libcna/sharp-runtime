...
dependency edges, a clean native build, 12,641 passing tests across 37
...
- Tests: 12,641 passing across 36 component binaries plus one integration
  binary.
...
| `ticket` | 1,753 rows, all `done`; no `todo`, `doing`, `blocked`, or `needs_user` rows |
...
1. **Continue a bounded `ImmutableList<T>` slice.**
   ...
   sort, and equality-comparer query overloads. Do not attempt the
