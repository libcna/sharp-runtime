...
repair, three P1 parity repairs, P1 portability revalidation, and fifteen bounded
P2 API slices are complete: 41 physical modules, 90 production dependency
edges, and 12,654 tests across 37 executables.*
...
- The full native baseline is a warning-free build with 12,654 passing tests
...
- `BigInteger` supports `&, |, ^, ~` and their compound assignments using
  ...
  integer values. Shift and byte-array conversion APIs remain deferred.
...
1,757 completed tickets. Ticket #1737 records the completed P0 split, tickets
...
range sort, and #1757 for `BigInteger` bitwise operators; it is git-ignored
...
2. **Other documented partial surfaces.** Examples include `BigInteger` shift
   or byte-array APIs, fuller UTF-7 behavior, and wider debugger/process/XML
   surfaces.
