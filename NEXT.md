...
repair, three P1 parity repairs, P1 portability revalidation, and sixteen bounded
P2 API slices are complete: 41 physical modules, 90 production dependency
edges, and 12,658 tests across 37 executables.*
...
- The full native baseline is a warning-free build with 12,658 passing tests
...
- `BigInteger` supports `&, |, ^, ~` and their compound assignments using
  ...
  byte-array conversion APIs remain deferred.
...
1,758 completed tickets. Ticket #1737 records the completed P0 split, tickets
...
range sort, #1757 for `BigInteger` bitwise operators, and #1758 for signed
`BigInteger` shifts; it is git-ignored and is not part of a fresh clone.
...
2. **Other documented partial surfaces.** Examples include `BigInteger`
   byte-array APIs, fuller UTF-7 behavior, and wider debugger/process/XML
   surfaces.
