...
*Last updated: 2026-07-04 (branch: feature/work) — 9073 tests passing*
...
### Tests
- **9073 tests passing** across 897 test suites. Zero failures.
...
- Collections.ObjectModel: 6 of 7 items reviewed/fixed — `Collection<T>` (real bounds validation, was silent UB), `ReadOnlyCollection<T>` (NotSupportedException on mutation, was std::runtime_error), `KeyedCollection<TKey,TItem>`, `ObservableCollection<T>`, `ReadOnlyDictionary<K,V>`, `ReadOnlyObservableCollection<T>` (see §3 for details). Remaining: `ReadOnlySet`
...
| Commit | Change |
|--------|--------|
| — | Port Collections.ObjectModel ReadOnlyObservableCollection (6 of 7 items): **the wrapper took its source ObservableCollection by const-ref (copying it) or by rvalue-ref (moving it), producing a private, disconnected copy** — mutating the original collection afterwards was invisible through the wrapper and fired no CollectionChanged/PropertyChanged events, defeating the entire purpose of an *observable* read-only view (contrast with .NET, where `ReadOnlyCollection<T>`/`ReadOnlyObservableCollection<T>` wrap the live underlying list by reference). Rewrote around a shared `std::shared_ptr<ObservableCollection<T>>` (same pattern as `ReadOnlyDictionary`) so the wrapper is a true live view; constructor now throws ArgumentNullException on a null source; int→intcs |
| — | Port Collections.ObjectModel ReadOnlyDictionary (5 of 7 items): indexer threw std::out_of_range instead of KeyNotFoundException; **the constructor didn't validate the wrapped shared_ptr, so a null dictionary silently segfaulted on first access** instead of throwing ArgumentNullException like .NET; added `Empty()` and the protected `Dictionary` accessor for parity. Deviation (consistent with `Dictionary<K,V>` itself not implementing `Generic::IDictionary<K,V>` — no concrete container in this codebase implements its generic interface): does not implement `Generic::IDictionary<TKey,TValue>`, so mutating members are simply absent rather than present-but-throwing, matching .NET's own direct-call surface (its mutators are only reachable through an explicit interface cast) |
| — | Port Collections.ObjectModel ObservableCollection (4 of 7 items): **the previous implementation overrode the public Add/Remove/Clear methods directly instead of the protected InsertItem/RemoveItem/ClearItems/SetItem hooks that .NET's ObservableCollection<T> actually overrides** — as a result `Insert()`/`RemoveAt()` silently bypassed CollectionChanged entirely, and the protected `SetItem` hook didn't exist at all (no Replace notification path); rewrote to override the four hooks like Collection<T>/KeyedCollection already correctly do, so Add/Remove/Clear/Insert/RemoveAt/Move/SetItem all notify uniformly; **the vector constructor called Add() in a loop, firing N spurious CollectionChanged events during construction** instead of directly populating the base list with no notifications (.NET's actual construction semantics); added the entirely-missing `PropertyChanged` (Count/Item[] via the existing `System::ComponentModel::INotifyPropertyChanged`), `CheckReentrancy`/`BlockReentrancy` (throws InvalidOperationException on reentrant mutation with 2+ subscribers, matching .NET's single-subscriber compatibility carve-out), and `Move` had **zero bounds validation (silent UB via `operator[]` on an out-of-range std::vector index)** — now throws ArgumentOutOfRangeException; int→intcs throughout |
| — | Port Collections.ObjectModel KeyedCollection (3 of 7 items): **InsertItem/SetItem had zero duplicate-key validation (silent overwrite of the key index instead of ArgumentException)** — .NET throws when a second item with an existing key is added or a replacement collides with a different item's key; the key-by-indexer threw std::out_of_range instead of KeyNotFoundException; added the entirely-missing protected `SetItem` override (key index went stale on replacement) and `ChangeItemKey` (lets a subclass notify the collection before mutating an item's key field, matching .NET's documented ChangeItemKey contract); int→intcs throughout. Deviation (consistent with every other generic container in this codebase): no injectable `IEqualityComparer<TKey>` — TKey's own operator==/std::hash is always used |
